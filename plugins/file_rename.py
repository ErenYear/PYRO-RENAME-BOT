...
async def rename_callback(bot, query): 
    ...
    try:
        _, type = query.data.split("_", 1)  
    except ValueError:
        return await query.answer("Invalid callback data format!", show_alert=True)

    file_name = query.message.text.split(":-")[1].strip()  # Strip extra spaces
    ...
    try:
        if type == "document":
            await query.message.reply_document(document=file_path, caption=caption, thumb=ph_path)
        elif type == "video": 
            await query.message.reply_video(video=file_path, caption=caption, thumb=ph_path, duration=duration)
        elif type == "audio": 
            await query.message.reply_audio(audio=file_path, caption=caption, thumb=ph_path, duration=duration)
    except Exception as e:          
        try: 
            os.remove(file_path)
            if ph_path:
